...
numeric, integer, and character values.
...
Tests were performed on an Amazon high-memory EC2 instance with 32\,GB RAM and four
Intel Xeon CPU X5550 cores operating at 2.67\,GHz, running Ubuntu GNU/Linux
11.04 and R version 2.12.1.  Except where indicated, the raw data file
was located in a RAM-based tmpfs file system to completely eliminate disk
performance effects.
...
set into R which always failed after about 16 minutes with the error:
\begin{lstlisting}
...
Frustrated, I took the advice of the American Statistical Association
...
The sqlite3 data load time without creating indices took about 18
minutes.
%real    19m53.965s
%user    17m55.440s
%sys     1m30.310s
...
on the year field took about 26 minutes.
%real    25m49.930s
%user    23m39.590s
%sys     2m59.050s
...
%> t1=proc.time();x = dbGetQuery(ontime, "select * from ontime where Year=2000"); print(proc.time()-t1)
%   user  system elapsed 
% 37.500   4.840  42.358 
%2nd time
%  user  system elapsed
...
Lazy frame
%   user  system elapsed
%  2.170   4.500   6.677  -- load time
%   user  system elapsed
%158.150  66.820 225.006  -- z = x[x[,"Year"]==2000,]

%require(lazy.frame)
%t1 = proc.time()
%x = lazy.frame("./mem/airlines.csv",header=TRUE)
%print(proc.time()-t1)
...
ontime <- dbConnect("SQLite", dbname = "ontime.sqlite3")
t1=proc.time();
z = dbGetQuery(ontime, "select * from ontime where Year=2000");
print(proc.time()-t1)
print(dim(z))
print(gc())
...
[1] 5683047      29
           used  (Mb) gc trigger   (Mb)  max used   (Mb)
Ncells   145399   7.8     350000   18.7    350000   18.7
Vcells 93905286 716.5  295160237 2251.9 293661922 2240.5
